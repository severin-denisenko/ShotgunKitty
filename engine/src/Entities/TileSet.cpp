...
#include "engine/Entities/TileSet.h"
#include "engine/Game.h"

#include <fstream>

namespace engine {
    Index& TileSet::operator()(Pos x, Pos y) {
        if (x >= width || y >= height)
    ...
    const Index& TileSet::operator()(Pos x, Pos y) const {
        if (x >= width || y >= height)
            throw std::runtime_error("Index out of bounds in TileMap");
    ...
    void TileSet::Resize(Pos w, Pos h) {
        map.resize(h);
